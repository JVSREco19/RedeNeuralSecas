...
def showRMSETaylorDiagrams(training_true_values, training_predicted_values, testing_true_values, testing_predicted_values, city_cluster_name, city_for_training, city_for_predicting, showImages):
    ...
    if   degrees ==  90: correlation_range = np.arange( 0, 1.1, 0.1)  # (1.1 is discarded)
    elif degrees == 180: correlation_range = np.arange(-1, 1.1, 0.1)  # (1.1 is discarded)
    ...
    if   degrees ==  90: ax.text(np.pi / 4.2, 0.26, 'Pearson Correlation Coefficient', rotation=-45, ha='center', va='center', color='blue')
